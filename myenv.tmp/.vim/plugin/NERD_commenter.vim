...
function! NERDComment(isVisual, type) range
    ...
    elseif a:type == 'title'
        call s:CreateTitle(firstLine, lastLine)
...
function! s:CreateTitle( firstLine, lastLine )

    let currentLine = a:firstLine
    while currentLine <= a:lastLine
        let line = getline( currentLine )
        let line_len = strlen( line )
        let pad = repeat( '*', 80 - line_len - 3 )
        call setline( currentLine, '# ' . line . ' ' . pad )
        let currentLine = currentLine + 1
    endwhile
...
" create title
nnoremap <silent> <plug>NERDCommenterTitle :call NERDComment(0, "title")<cr>
vnoremap <silent> <plug>NERDCommenterTitle <ESC>:call NERDComment(1, "title")<CR>
...
if g:NERDCreateDefaultMappings
    ...
    call s:CreateMaps('<plug>NERDCommenterTitle',      '<leader>ct')
